package com.sulake.habbo.toolbar
{
   ...
   public class ExtensionView implements IExtensionView
   {
      ...
      private var var_137:IItemListWindow;
      ...
      public function ExtensionView(param1:IHabboWindowManager, param2:IAssetLibrary, param3:HabboToolbar)
      {
         ...
         if(_loc4_)
         {
            this.var_137 = param1.buildFromXML(_loc4_.content as XML,1) as IItemListWindow;
         }
         var _loc5_:IWindowContainer = this.var_137.desktop;
         if(this.var_137)
         {
            this.var_137.x = _loc5_.width - this.var_137.width - const_1093;
            this.var_137.y = const_1093;
            this.var_137.visible = true;
         }
         else
         {
            Core.error("Unable to initialize Toolbar Extension view window from xml asset",false,Core.const_1303);
         }
         this._items = new Map();
      }
      ...
      public function get screenHeight() : uint
      {
         ...
         return this.var_137.height + this.var_137.y;
      }
      ...
      public function attachExtension(param1:String, param2:IWindow, param3:int = -1) : void
      {
         ...
         if(this.var_137)
         {
            if(param3 == -1)
            {
               this.var_137.addListItem(param2);
            }
            else
            {
               this.var_137.addListItemAt(param2,param3);
            }
            this.var_137.invalidate();
         }
      ...
      public function detachExtension(param1:String) : void
      {
         ...
         if(this.var_137)
         {
            this.var_137.removeListItem(_loc2_);
         }
      ...
      private function onResizeTimer(param1:TimerEvent) : void
      {
         if(this._toolbar)
         {
            this._toolbar.events.dispatchEvent(new ExtensionViewEvent(ExtensionViewEvent.const_1417));
         }
      }
   }
}
