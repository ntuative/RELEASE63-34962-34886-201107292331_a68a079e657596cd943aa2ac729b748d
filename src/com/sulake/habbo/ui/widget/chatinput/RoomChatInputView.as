package com.sulake.habbo.ui.widget.chatinput
{
   ...
   public class RoomChatInputView
   {
      ...
      private static const const_1487:int = 250;
      
      private static const const_1486:int = 0;
      
      private static const BOTTOM_MARGIN:int = 55;
      
      private static const const_1488:String = "${widgets.chatinput.say}";
      ...
      public function RoomChatInputView(param1:RoomChatInputWidget)
      {
         super();
         this._widget = param1;
         this.var_1205 = param1.localizations.getKey("widgets.chatinput.mode.whisper",":tell");
         ...
         this.var_380 = new Timer(1000,1);
         this.var_380.addEventListener(TimerEvent.TIMER_COMPLETE,this.onTypingTimerComplete);
         this.var_459 = new Timer(10000,1);
         this.var_459.addEventListener(TimerEvent.TIMER_COMPLETE,this.onIdleTimerComplete);
         this.createWindow();
      }
      ...
      public function dispose() : void
      {
         ...
         {
            this.var_380.reset();
            this.var_380.removeEventListener(TimerEvent.TIMER_COMPLETE,this.onTypingTimerComplete);
            this.var_380 = null;
         }
         if(this.var_459 != null)
         {
            this.var_459.reset();
            this.var_459.removeEventListener(TimerEvent.TIMER_COMPLETE,this.onIdleTimerComplete);
            this.var_459 = null;
         }
      }
      ...
      private function createWindow() : void
      {
         ...
         this.var_1825 = this._window.findChildByName("input_border");
         this._button = this._window.findChildByName("send_button");
         this.var_1824 = this._window.findChildByName("send_button_text");
         this.var_1491 = this._window.findChildByName("block_text");
         this.var_1824.caption = const_1488;
         this.var_26.setParamFlag(HabboWindowParam.const_43,true);
         this.var_26.addEventListener(WindowMouseEvent.const_44,this.windowMouseEventProcessor);
         this.var_26.addEventListener(WindowKeyboardEvent.const_194,this.windowKeyEventProcessor);
         ...
         this.var_1490 = true;
         this.var_638 = "";
         this._window.addEventListener(WindowEvent.const_569,this.updatePosition);
         this._window.addEventListener(WindowEvent.const_625,this.updatePosition);
      }
      ...
      private function updatePosition(param1:WindowEvent = null) : void
      {
         ...
         var _loc4_:int = (this._window.desktop.width - this.var_1825.width) / 2 - _loc3_;
         var _loc5_:int = this._window.desktop.width - this._window.width - const_1487 - _loc3_;
         this._window.x = Math.max(Math.min(_loc4_,_loc5_),const_1486);
      }
      ...
      private function windowKeyEventProcessor(param1:WindowEvent = null, param2:IWindow = null) : void
      {
         ...
         if(_loc3_ == Keyboard.BACKSPACE)
         {
            if(this.var_26 != null)
            {
               ...
               _loc8_ = _loc7_.split(" ");
               if(_loc8_[0] == this.var_1205 && _loc8_.length == 3 && _loc8_[2] == "")
               {
                  this.var_26.text = "";
                  this.var_638 = "";
               }
      ...
      private function onInputChanged(param1:WindowEvent) : void
      {
         ...
         {
            if(_loc2_.text.length > this.var_638.length + 1)
            {
      ...
      private function sendTypingMessage() : void
      {
         ...
         var _loc1_:RoomWidgetChatTypingMessage = new RoomWidgetChatTypingMessage(this.var_639);
      ...
      private function sendChatFromInputField(param1:Boolean = false) : void
      {
         ...
         if(this._widget != null)
         {
            if(this.var_380.running)
            {
               this.var_380.reset();
            }
            if(this.var_459.running)
            {
               this.var_459.reset();
            }
