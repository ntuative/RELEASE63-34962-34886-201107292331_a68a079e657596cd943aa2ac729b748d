package com.sulake.habbo.sound.trax
{
   ...
   public class TraxSequencer implements IHabboSound, IDisposable
   {
      ...
      private static const const_1057:uint = 88000;
      
      private static const const_686:uint = 88000;
      
      private static const const_200:Vector.<int> = new Vector.<int>(const_119,true);
      ...
      private var var_960:Sound;
      
      private var var_443:SoundChannel;
      
      private var var_445:TraxData;
      ...
      {
         this.var_1449 = uint(30);
         super();
         this._events = param4;
         this.var_1445 = param1;
         this.var_1049 = 1;
         this.var_960 = new Sound();
         this.var_443 = null;
      ...
      {
         this.var_171 = uint(param1 * const_118);
      }
      ...
      {
         return this.var_446 / const_118;
      }
      
      public function set fadeOutSeconds(param1:Number) : void
      {
         this.var_446 = int(param1 * const_118);
      }
      
      public function get fadeInSeconds() : Number
      {
         return this.var_771 / const_118;
      }
      
      public function set fadeInSeconds(param1:Number) : void
      {
         this.var_771 = int(param1 * const_118);
      }
      ...
      public function prepare() : Boolean
      {
         ...
         if(!this.var_1448)
         {
            ...
            {
               this.var_1446 = false;
               if(this.var_445.hasMetaData)
               {
                  this.var_1446 = this.var_445.metaCutMode;
               }
               if(this.var_1446)
               {
      ...
      private function prepareLegacySequence() : Boolean
      {
         ...
         var _loc2_:int = 0;
         while(_loc2_ < this.var_445.channels.length)
         {
            _loc3_ = new Map();
            _loc4_ = this.var_445.channels[_loc2_] as TraxChannel;
            _loc5_ = 0;
            _loc6_ = 0;
            _loc7_ = 0;
            while(_loc7_ < _loc4_.itemCount)
            {
               ...
               _loc9_ = this.var_1447.getValue(_loc8_) as TraxSample;
               _loc9_.setUsageFromSong(this.var_1445,_loc1_);
               if(_loc9_ == null)
               {
               ...
               while(_loc12_ < _loc11_)
               {
                  ...
                  _loc6_ += _loc10_;
                  _loc5_ = uint(_loc6_ * const_686);
                  _loc12_++;
               }
               if(this.var_272 < _loc5_)
               {
                  this.var_272 = _loc5_;
               }
               _loc7_++;
      ...
      private function prepareSequence() : Boolean
      {
         ...
         while(_loc2_ < this.var_445.channels.length)
         {
            ...
            while(_loc8_ < _loc4_.itemCount)
            {
               ...
               _loc10_ = this.var_1447.getValue(_loc9_) as TraxSample;
               _loc10_.setUsageFromSong(this.var_1445,_loc1_);
               if(_loc10_ == null)
               {
      ...
      public function play(param1:Number = 0.0) : Boolean
      {
         ...
         this.var_525 = 0;
         this.var_624 = false;
         this.var_960.addEventListener(SampleDataEvent.SAMPLE_DATA,this.onSampleData);
         this.var_1450 = param1 * const_118;
         this.var_1175 = 0;
         this.var_1745 = 0;
         this.var_443 = this.var_960.play();
      ...
      {
         if(this.var_446 > 0 && !this.var_624)
         {
      ...
      private function stopImmediately() : void
      {
         ...
         {
            this.var_960.removeEventListener(SampleDataEvent.SAMPLE_DATA,this.onSampleData);
         }
      }
      ...
      private function stopWithFadeout() : void
      {
         if(this.var_623 == null)
         {
            this.var_772 = true;
            this.var_525 = 0;
            this.var_623 = new Timer(this.var_1449 + this.var_446 / (const_118 / 1000),1);
            this.var_623.start();
            this.var_623.addEventListener(TimerEvent.TIMER_COMPLETE,this.onFadeOutComplete);
         }
      }
      ...
      private function getSampleBars(param1:uint) : int
      {
         if(this.var_1446)
         {
            return Math.round(param1 / const_1057);
         }
         return Math.ceil(param1 / const_1057);
      }
      ...
      private function getChannelSequenceOffsets() : Array
      {
         ...
         if(this.var_322 != null)
         {
            _loc2_ = this.var_322.length;
            _loc3_ = 0;
            while(_loc3_ < _loc2_)
            {
               _loc4_ = this.var_322[_loc3_];
               _loc5_ = 0;
               while(_loc5_ < _loc4_.length && _loc4_.getKey(_loc5_) < this.var_171)
               {
                  _loc5_++;
               }
      ...
      private function mixChannelsIntoBuffer() : void
      {
         ...
         var _loc2_:int = this.var_322.length;
         var _loc3_:* = null;
         var _loc4_:int = _loc2_ - 1;
         while(_loc4_ >= 0)
         {
            _loc5_ = this.var_322[_loc4_];
            ...
            _loc8_ = const_119;
            if(this.var_272 - this.var_171 < _loc8_)
            {
               _loc8_ = this.var_272 - this.var_171;
            }
            _loc9_ = 0;
            while(_loc9_ < _loc8_)
            {
               ...
               if(_loc6_ < _loc5_.length - 1)
               {
                  ...
                  if(_loc8_ + this.var_171 >= _loc13_)
                  {
                     _loc12_ = _loc13_ - this.var_171;
                  }
               }
      ...
      {
         var _loc1_:int = this.var_272 < this.var_1450 ? int(this.var_272) : (this.var_1450 > 0 ? int(this.var_1450) : int(this.var_272));
         if(this.var_171 > _loc1_ + this.var_1449 * (const_118 / 1000) && !this.var_624)
         {
            this.var_624 = true;
            if(this.var_444 != null)
            {
               this.var_444.reset();
               this.var_444.removeEventListener(TimerEvent.TIMER_COMPLETE,this.onPlayingComplete);
            }
            this.var_444 = new Timer(2,1);
            this.var_444.start();
            this.var_444.addEventListener(TimerEvent.TIMER_COMPLETE,this.onPlayingComplete);
         }
         else if(this.var_171 > _loc1_ - this.var_446 && !this.var_772)
         {
            this.var_961 = false;
            this.var_772 = true;
            this.var_525 = 0;
      ...
      {
         if(param1.position > this.var_1175)
         {
            ++this.var_1745;
            Logger.log("Audio buffer under run...");
            this.var_1175 = param1.position;
         }
         if(this.volume > 0)
         {
         ...
         if(this.var_272 - this.var_171 < _loc2_)
         {
            _loc2_ = this.var_272 - this.var_171;
            if(_loc2_ < 0)
            {
               _loc2_ = 0;
            }
         ...
         this.var_171 += const_119;
         this.var_1175 += const_119;
         if(this.var_443 != null)
         {
            this.var_1449 = param1.position / const_118 * 1000 - this.var_443.position;
         }
      ...
      private function writeAudioToOutputStream(param1:ByteArray, param2:int) : void
      {
         ...
         if(param2 > 0)
         {
            ...
            else
            {
               if(this.var_961)
               {
                  _loc5_ = 1 / this.var_771;
                  _loc6_ = this.var_962 / Number(this.var_771);
                  this.var_962 += const_119;
                  if(this.var_962 > this.var_771)
                  {
                     this.var_961 = false;
                  }
               }
               else if(this.var_772)
               {
                  _loc5_ = -1 / this.var_446;
                  _loc6_ = 1 - this.var_525 / Number(this.var_446);
                  this.var_525 += const_119;
                  if(this.var_525 > this.var_446)
                  {
                     this.var_525 = this.var_446;
                  }
               }
      ...
      {
         this.stopImmediately();
         this._events.dispatchEvent(new SoundCompleteEvent(SoundCompleteEvent.TRAX_SONG_COMPLETE,this.var_1445));
      }
      ...
      private function removeFadeoutStopTimer() : void
      {
         if(this.var_623 != null)
         {
            this.var_623.removeEventListener(TimerEvent.TIMER_COMPLETE,this.onFadeOutComplete);
            this.var_623.reset();
            this.var_623 = null;
         }
      }
