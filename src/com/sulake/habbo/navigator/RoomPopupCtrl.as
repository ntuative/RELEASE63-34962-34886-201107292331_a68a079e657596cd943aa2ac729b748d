package com.sulake.habbo.navigator
{
   ...
   public class RoomPopupCtrl extends PopupCtrl
   {
      ...
      private var var_2513:RoomThumbnailRenderer;
      ...
      public function RoomPopupCtrl(param1:HabboNavigator, param2:int, param3:int)
      {
         ...
         this.var_2513 = new RoomThumbnailRenderer(param1);
         this.var_986 = new TagRenderer(param1,hideInstantly);
      }
      ...
      override public function refreshContent(param1:IWindowContainer) : void
      {
         ...
         {
            this.var_200 = IWindowContainer(param1.findChildByName("details_container"));
         }
         this.var_200.visible = true;
         Util.hideChildren(this.var_200);
         this.refreshRoomName(this.var_200,this.var_140);
         if(this.var_140.ownerName != "" && this.var_140.ownerName != "-")
         {
         }
         this.refreshOwnerName();
         this.refreshTextWithCaption("roomctg",this.var_200,!!this.var_140.event ? this.getEventCtg(this.var_140.categoryId) : this.getRoomCtg(this.var_140.categoryId));
         this.refreshRoomDesc(this.var_200,this.var_140);
         this.refreshExtraCont();
         this.refreshInfo(this.var_200,"trading_allowed",this.var_140.allowTrading);
         this.refreshInfo(this.var_200,"doormode_doorbell",this.var_140.doorMode == 1);
         this.refreshInfo(this.var_200,"doormode_password",this.var_140.doorMode == 2);
         Util.moveChildrenToColumn(this.var_200,["roomname","roomctg_cont","roomowner_cont","roomdesc","extra_cont","doormode_doorbell","doormode_password","trading_allowed"],0,0);
         this.var_200.height = Util.getLowestPoint(this.var_200);
      }
      
      private function refreshOwnerName() : void
      {
         this.refreshTextWithCaption("roomowner",this.var_200,this.var_140.ownerName);
         var _loc1_:ITextWindow = ITextWindow(this.var_200.findChildByName("roomowner"));
         if(this.var_140.ownerName != "" && this.var_140.ownerName != "-")
         {
      ...
      {
         var _loc1_:IWindowContainer = IWindowContainer(this.var_200.findChildByName("extra_cont"));
         Util.hideChildren(_loc1_);
         this.var_986.refreshTags(_loc1_,this.var_140.tags);
         ...
         this.var_2513.refreshThumbnail(_loc1_,this.var_140.thumbnail,false);
      ...
      private function onOwnerName(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(_loc3_ != null)
         {
            navigator.mainViewCtrl.startSearch(Tabs.const_282,Tabs.const_415,_loc3_.text);
            this.hideInstantly();
         }
      }
   }
