package com.sulake.habbo.communication.demo
{
   ...
   public class HabboCommunicationDemo extends Component
   {
      ...
      private var var_593:IKeyExchange;
      
      private var var_1106:String;
      
      private var _view:HabboLoginDemoView;
      
      private var var_2909:PseudoRandom;
      ...
      private const const_2097:String = "fuselogin";
      ...
      public function setSSOTicket(param1:String) : void
      {
         ...
         {
            this.var_724 = param1;
            this.var_66.initConnection(HabboConnectionType.const_244);
         }
      }
      ...
      private function onRoomSessionManagerReady(param1:IID, param2:IUnknown) : void
      {
         ...
         this.var_15.events.addEventListener(RoomSessionEvent.const_85,this.onRoomSessionEnded);
         this.checkRequirements();
      }
      ...
      {
         if(this.var_66 && this.var_161 && this.var_15 && this._windowManager)
         {
      ...
      private function componentsAreRunning() : void
      {
         ...
         var _loc1_:Boolean = this.var_161.getBoolean("client.hotel_view.show_on_startup",true);
         this.var_258 = new HabboHotelView(this._windowManager,assets,Component(context).events,_loc1_);
         var _loc2_:String = this.var_161.getKey("client.hotel_view.image");
         var _loc3_:String = this.var_161.getKey("image.library.url");
         if(_loc2_ != null && _loc3_ != null)
         {
            this.var_258.loadHotelViewImage(_loc3_ + _loc2_);
         }
         var _loc4_:* = this.var_161.getKey("use.sso.ticket") == "1";
         this.var_724 = this.var_161.getKey("sso.ticket",null);
         this.var_2123 = this.var_161.getKey("flash.client.url","");
         this.var_2907 = this.var_161.getKey("external.variables.txt","");
         if(this.var_258 == null)
         {
            this.var_258 = new HabboHotelView(this._windowManager,assets,Component(context).events,_loc1_);
         }
         if(_loc4_)
         {
            this.var_66.mode = HabboConnectionType.const_98;
            if(this.var_724)
            {
               this.var_66.initConnection(HabboConnectionType.const_244);
            }
         ...
         else if(this._windowManager != null)
         {
            ...
            if(_loc5_)
            {
               ...
               this._view.addEventListener(HabboLoginDemoView.const_850,this.onInitConnection);
            }
            else
            {
               Core.crash("Login without an SSO ticket is not supported",Core.const_1985);
            }
         }
      }
      ...
      private function onInitConnection(param1:Event = null) : void
      {
         ...
         {
            this.var_66.mode = HabboConnectionType.const_98;
            if(this.var_161.keyExists("local.environment"))
            {
               this.var_66.mode = HabboConnectionType.const_1366;
            }
         }
         this.var_66.initConnection(HabboConnectionType.const_244);
      }
      ...
      private function onConnectionEstablished(param1:Event = null) : void
      {
         ...
         if(_loc2_ != null)
         {
            this.dispatchLoginStepEvent(HabboCommunicationEvent.const_989);
      ...
      private function onInitCrypto(param1:IMessageEvent) : void
      {
         ...
         this.var_2909 = new PseudoRandom(parseInt(_loc4_.substring(_loc4_.length - 4),16),65536);
         switch(this.var_66.mode)
         {
            case HabboConnectionType.const_98:
               _loc6_ = "";
               _loc6_ = this.var_161.getKey("url.prefix",_loc6_);
               _loc6_ = _loc6_.replace("http://","");
               _loc6_ = _loc6_.replace("https://","");
               _loc7_ = this.var_161.getKey("hotelview.banner.url","http:/\nsitename$/gamedata/banner");
               _loc7_ = _loc7_.replace("$sitename$",_loc6_);
               this.var_1643 = _loc4_;
               this.var_258.loadBannerImage(_loc7_ + "?token=" + this.var_1643,this.onHotelViewBannerLoaded);
               break;
            case HabboConnectionType.const_1366:
               this.sendConnectionParameters(_loc2_);
               break;
            case HabboConnectionType.const_2056:
               this.var_1106 = this.generateRandomHexString(30);
               this.var_593 = new DiffieHellman(new BigInteger(this.var_161.getKey("connection.development.prime"),16),new BigInteger(this.var_161.getKey("connection.development.generator"),16));
               this.var_593.init(this.var_1106);
               _loc8_ = this.var_593.getPublicKey(10);
      ...
      private function onSecretKeyEvent(param1:IMessageEvent) : void
      {
         ...
         this.var_593.generateSharedKey(_loc4_,10);
         var _loc5_:String = this.var_593.getSharedKey(16).toUpperCase();
         var _loc6_:RC4_R27 = new RC4_R27(null,null);
         var _loc7_:IEncryption = new RC4_R27(_loc6_,this.var_2909);
      ...
      private function sendConnectionParameters(param1:IConnection) : void
      {
         ...
         this.var_1644 = true;
         this.dispatchLoginStepEvent(HabboCommunicationEvent.const_284);
         connection.send(new VersionCheckMessageComposer(401,this.var_2123,this.var_2907));
         var machineId:String = "";
         try
         {
            so = SharedObject.getLocal(this.const_2097,"/");
      ...
      private function onSessionParamsEvent(param1:IMessageEvent) : void
      {
         ...
         if(this.var_724 != null)
         {
            _loc4_ = new SSOTicketMessageComposer(this.var_724);
            _loc2_.send(_loc4_);
            this.dispatchLoginStepEvent(HabboCommunicationEvent.const_218);
         }
      ...
      private function onConnectionDisconnected(param1:Event) : void
      {
         ...
         if(param1.type == Event.CLOSE && !this.var_2908)
         {
            _loc2_ = this.var_161.getKey("logout.disconnect.url");
      ...
      private function onDisconnectReason(param1:DisconnectReasonEvent) : void
      {
         ...
         this.var_2908 = true;
         var _loc2_:String = this.var_161.getKey("logout.url");
         if(_loc2_.length > 0)
         {
      ...
      private function setOriginProperty(param1:String) : String
      {
         ...
         {
            return param1.replace("%origin%",this.var_161.getKey("flash.client.origin","popup"));
         }
         return param1;
      }
      ...
      private function onHotelViewBannerLoaded(param1:BitmapData) : void
      {
         ...
         var _loc4_:String = xor(_loc3_,this.var_1643);
         ...
         this.var_593 = new DiffieHellman(_loc10_,_loc11_);
         var _loc13_:int = 10;
         var _loc14_:* = null;
         while(_loc13_ > 0)
         {
            _loc14_ = this.generateRandomHexString(30);
            this.var_593.init(_loc14_);
            _loc15_ = this.var_593.getPublicKey(10);
