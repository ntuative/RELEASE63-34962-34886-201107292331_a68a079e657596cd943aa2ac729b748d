package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private var var_1083:int = 3;
      
      private var var_494:AssetLoaderStruct;
      
      private var var_1084:Timer;
      ...
      private const const_2386:int = 100;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         if(_loc6_ == null)
         {
            this.var_494 = this._assets.loadAssetFromFile("figuremap",_loc5_,"text/xml");
         ...
         this.var_1084 = new Timer(this.const_2386,1);
         this.var_1084.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_494)
         {
            this.var_494.addEventListener(AssetLoaderEvent.ASSET_LOADER_EVENT_COMPLETE,this.onConfigurationComplete);
            this.var_494.addEventListener(AssetLoaderEvent.const_40,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_494)
         {
            this.var_494.removeEventListener(AssetLoaderEvent.ASSET_LOADER_EVENT_COMPLETE,this.onConfigurationComplete);
            this.var_494.removeEventListener(AssetLoaderEvent.const_40,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         if(this.var_1083 <= 0)
         {
            HabboWebTools.logEventLog("Figuremap download error " + this.var_1327);
            Logger.log("AVATAR ASSET ERROR: Could not download figuremap.xml.");
         }
         else
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_1083);
            if(this.var_1327.indexOf("?") > 0)
            {
               _loc2_ = this.var_1327 + "&retry=" + this.var_1083;
            }
            else
            {
               _loc2_ = this.var_1327 + "?retry=" + this.var_1083;
            ...
            this.var_494 = this._assets.loadAssetFromFile("figuremap",_loc3_,"text/xml");
            this.addMapLoaderEventListeners();
         }
      }
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2851,this._assets);
            _loc3_.addEventListener(Event.COMPLETE,this.libraryComplete);
            for each(_loc4_ in _loc2_.part)
            {
               _loc5_ = _loc4_.@type + ":" + _loc4_.@id;
               _loc6_ = this.var_1328[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(_loc3_);
               this.var_1328[_loc5_] = _loc6_;
            }
         }
      }
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_2080)
         {
            this.var_1326.push([param1,param2]);
            return;
         }
         ...
         if(_loc4_.length > 0)
         {
            ...
            {
               _loc6_ = this.var_566[_loc3_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(param2);
               this.var_566[_loc3_] = _loc6_;
            }
            this.var_1082[_loc3_] = _loc4_;
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         while(_loc6_ < this.var_867.length)
         {
            _loc5_ = this.var_867[_loc6_];
            if(_loc5_.libraryName == (param1.target as AvatarAssetDownloadLibrary).libraryName)
            {
               this.var_867.splice(_loc6_,1);
            }
            _loc6_++;
         }
         this.var_1084.start();
      ...
      {
         var _loc1_:* = null;
         while(this.var_868.length > 0 && this.var_867.length < this.const_1866)
         {
            _loc1_ = this.var_868[0];
            _loc1_.startDownloading();
            this.var_867.push(this.var_868.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_868.indexOf(param1) == -1 && this.var_867.indexOf(param1) == -1)
         {
            this.var_868.push(param1);
            this.processPending();
         }
      }
