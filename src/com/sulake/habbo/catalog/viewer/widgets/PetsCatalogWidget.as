package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IPetDataListener, IPetImageListener
   {
      ...
      override public function init() : Boolean
      {
         ...
         this.var_2621 = _loc3_.localizationId;
         this.var_393 = [];
         if(page && false && page.viewer.catalog)
         {
      ...
      private function onWidgetsInitialized(param1:CatalogWidgetsInitializedEvent = null) : void
      {
         ...
         var _loc3_:* = [];
         for each(_loc4_ in this.var_393)
         {
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         this.var_556 = param1.index;
         if(this.var_556 < 0 || this.var_556 > this.var_393.length)
         {
            this.var_556 = 0;
         }
         this.updateImage();
      ...
      {
         if(param1.productCode == this.var_2621)
         {
      ...
      private function updateImage() : void
      {
         ...
         if(this.var_221 == null || this.var_830 >= this.var_221.length)
         {
            return;
         }
         var _loc2_:* = 0;
         if(this.var_556 >= 0 && this.var_556 < this.var_393.length)
         {
            _loc7_ = this.var_393[this.var_556] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:int = this.var_221[this.var_830] as int;
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(this.var_564,_loc3_,_loc2_,AvatarScaleType.const_60,this);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_346,AvatarAction.const_921);
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(this.var_556 >= this.var_393.length)
         {
            return "";
         }
         var petColor:IPetColor = this.var_393[this.var_556] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:int = this.var_221[this.var_830] as int;
