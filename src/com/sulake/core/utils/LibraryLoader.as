package com.sulake.core.utils
{
   ...
   public class LibraryLoader extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_2082:uint = 1;
      
      protected static const const_2084:uint = 2;
      
      protected static const const_2083:uint = 4;
      
      protected static const const_2085:uint = 3;
      ...
      public static const const_1253:Boolean = false;
      
      public static const const_1866:int = 2;
      
      private static var var_1128:Array = new Array();
      ...
      protected var var_270:URLRequest;
      
      protected var _manifest:XML;
      
      protected var var_702:Class;
      
      protected var var_519:uint = 0;
      ...
      protected var var_3142:uint = 0;
      
      protected var var_3078:Boolean = true;
      ...
      protected static function queue(param1:LibraryLoader) : void
      {
         ...
         {
            param1.var_44.load(param1.var_270,param1._context);
         }
      }
      ...
      protected static function throttle() : void
      {
         ...
         if(const_1253)
         {
            ...
            while(_loc2_ > -1)
            {
               _loc1_ = var_913[_loc2_] as LibraryLoader;
               if(_loc1_ && (_loc1_.ready || _loc1_.disposed))
               {
                  var_913.splice(_loc2_,1);
               }
               _loc2_--;
            }
            while(var_913.length < const_1866 && false)
            {
               _loc1_ = var_1128.shift() as LibraryLoader;
               if(!_loc1_.ready && !_loc1_.disposed)
               {
                  var_913.push(_loc1_);
                  _loc1_.var_44.load(_loc1_.var_270,_loc1_._context);
               }
            }
         }
      }
      ...
      {
         return !!this.var_270 ? this.var_270.url : null;
      }
      ...
      {
         return !!this.var_1162 ? uint(this.var_1435 - this.var_2287) : uint(getTimer() - this.var_2287);
      }
      ...
      override public function dispose() : void
      {
         if(!disposed)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DISPOSE,this.var_439,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
            try
            {
               this.var_44.close();
            }
      ...
      {
         this.var_270 = param1;
         this._name = parseNameFromUrl(this.var_270.url);
         this.var_1162 = false;
      ...
      {
         if(this.var_1161 && !_disposed)
         {
            this.var_1161 = false;
            if(!this.var_1162 && this.var_270)
            {
               this.var_1435 = -1;
      ...
      {
         if(!this.var_1162 && !_disposed && this.var_1434 > 0)
         {
            try
            {
               this.var_44.close();
            ...
            this.addRequestCounterToUrlRequest(this.var_270,const_1806 - this.var_1434);
            if(this.var_3078)
            {
               this.var_270.requestHeaders.push(new URLRequestHeader("pragma","no-cache"));
               this.var_270.requestHeaders.push(new URLRequestHeader("Cache-Control","no-cache"));
               this.var_3078 = false;
            }
            --this.var_1434;
            this.var_519 = 0;
            this.var_44.load(this.var_270,this._context);
            return true;
         }
         return false;
      }
      ...
      protected function loadEventHandler(param1:Event) : void
      {
         ...
         switch(param1.type)
         {
            ...
            case HTTPStatusEvent.HTTP_STATUS:
               this.var_439 = HTTPStatusEvent(param1).status;
               this.debug("Load event STATUS " + this.var_439 + " for file \"" + this.url + "\"");
               break;
            case Event.UNLOAD:
               this.debug("Load event UNLOAD for file \"" + this.url + "\"");
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_UNLOAD,this.var_439,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
               break;
            case ProgressEvent.PROGRESS:
               this.debug("Load event PROGRESS for file \"" + this.url + "\"  bytes: " + this.bytesLoaded + "/" + this.bytesTotal);
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_PROGRESS,this.var_439,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
               break;
            case IOErrorEvent.IO_ERROR:
               this.debug("Load event IO ERROR for file \"" + this.url + "\"");
               if(!this.handleHttpStatus(this.var_439))
               {
            ...
            case TimerEvent.TIMER:
               ...
               if(!_disposed)
               {
                  this.var_519 = LibraryLoader.name_5;
         ...
         if(this.var_519 == LibraryLoader.const_2085)
         {
         ...
         if(this.var_519 == LibraryLoader.name_5)
         {
            if(this.prepareLibrary())
            {
               ...
               this.removeEventListeners();
               throttle();
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE,this.var_439,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
      ...
      protected function prepareLibrary() : Boolean
      {
         ...
         this.var_702 = this.getDefinition(this._name) as Class;
         if(this.var_702 == null)
         {
            if(!this.retry())
            {
               this.failure("Failed to find resource class \"" + this._name + "\" from library " + this.var_270.url + "!");
            }
            return false;
         }
         ...
         try
         {
            xmlClass = this.var_702.manifest as Class;
            if(xmlClass == null)
            {
               return false;
            }
      ...
      protected function debug(param1:String) : void
      {
         ...
         if(this._debug)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DEBUG,this.var_439,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
         }
      }
      ...
      protected function failure(param1:String) : void
      {
         ...
         throttle();
         dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR,this.var_439,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
         dispatchEvent(new Event(LIBRARY_LOADER_FINALIZE));
      }
